    ...
    success: function(response) {
      ...
      if (response.length == 0) {
        ...
        for (var i = 0; i < response.length; i++) {
          ...
          var profile = getChildElement(response[i].fullName, response[i].designation, response[i].country)
          profile.setAttribute('onclick', "openMentorProfile('"+response[i].uid+"')" )
