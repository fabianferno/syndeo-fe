  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      document.getElementById("formTitle").innerHTML =
        (response.isMentor == 1 ? "Mentor" : "Student") +
        "'s Basic information";
...
$(document).ready(() => {
  ...
  for (var i = 0; i < years; i++) {
    if (i == years - 1 && new Date().getMonth() < 6) break;
  ...
  document.getElementById("editProfileButton").addEventListener("click", () => {
    ...
    document
      .getElementById("editProfileButtonLoader")
    ...
    let higherStudies = [];
    Array.from(document.getElementsByClassName("higherStudies")).forEach(
      (field, i) => {
        if (field.value != "" && i != 0) higherStudies.push(field.value);
      }
    );
    higherStudies.length == 0 ? null : higherStudies.join(",");

    let licenseAndCerts = [];
    Array.from(document.getElementsByClassName("licenseAndCerts")).forEach(
      (field, i) => {
        if (field.value != "" && i != 0) licenseAndCerts.push(field.value);
      }
    );
...
function setDefaultAvatar() {
  ...
  var profileAvatar = document.getElementById("profilePic");
...
function addFields(id) {
  ...
  inputField.lastChild.previousSibling.setAttribute(
    "onclick",
    'removeFields("' + newId + '")'
  );
}
